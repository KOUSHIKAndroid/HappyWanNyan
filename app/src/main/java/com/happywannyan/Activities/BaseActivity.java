        ...
        new AppConstant(BaseActivity.this).getShareData(AppDataHolder.UserData, new AppDataHolder.AppSharePreferenceDataInterface() {
            ...
            public void available(boolean available, JSONObject data) {
                try {
                    ...
                    if(!data.getJSONObject("info_array").getString("photo").trim().equals("")){
                        Glide.with(BaseActivity.this).load(data.getJSONObject("info_array").getString("photo").trim()).placeholder(R.drawable.ic_user).transform(new CircleTransform(BaseActivity.this)).into(UserImage);
                    }
